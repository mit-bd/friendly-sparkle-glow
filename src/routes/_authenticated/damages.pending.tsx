          ...
          rows.map((r) => {
            ...
            return (
              <MobileRecordCard key={r.id}
                title={r.damage_number}
                trailing={formatTk(r.damage_value)}
                subtitle={r.product_name || "—"}
                footer={<><StatusBadge status={r.status} /><LossApprovalPanel kind="damage" record={{ id: r.id, number: r.damage_number, status: r.status }} onDone={load} /></>}
                onClick={() => navigate({ to: "/damages/$id", params: { id: r.id } })}
                swipeActions={[{ label: "Review", icon: Eye, tone: "brand", onClick: () => navigate({ to: "/damages/$id", params: { id: r.id } }) }]}
                details={[
                  { label: "Date", value: formatDate(r.damage_date) },
