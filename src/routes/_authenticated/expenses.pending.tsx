      ...
      <div className="space-y-3 md:hidden">
        {loading ? (
          ...
          <Card><CardContent className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">{search || statusFilter !== ALL || categoryFilter !== ALL ? "Try adjusting your search or filters." : "Submitted expenses will appear here for review."}</p>
          </CardContent></Card>
        ) : (
          rows.map((r) => {
            ...
            return (
              <MobileRecordCard key={r.id}
                ...
                subtitle={`${r.category_id ? catMap.get(r.category_id) ?? "—" : "—"}${r.subcategory_id ? ` · ${subMap.get(r.subcategory_id) ?? ""}` : ""}`}
                footer={<><StatusBadge status={r.status} /><ApprovalPanel expense={r} onDone={load} compact /></>}
                onClick={() => navigate({ to: "/expenses/$id", params: { id: r.id } })}
                swipeActions={[{ label: "Review", icon: Eye, tone: "brand", onClick: () => navigate({ to: "/expenses/$id", params: { id: r.id } }) }]}
                details={[
