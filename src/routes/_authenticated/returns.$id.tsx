...
import { ATTACHMENT_BUCKET, fetchCategories, fetchUserNames, formatDate, formatDateTime, type ExpenseCategory } from "@/lib/expenses";
import type { ExpenseEvent } from "@/lib/approvals";
import {
  addReturnAttachment, deleteLossAttachment, fetchReturn, fetchReturnAttachments, fetchReturnEvents, fetchReturnReasons,
  formatTk, setLossStatus, updateReturn, type LossAttachment, type LossEvent, type ReturnReason, type ReturnRecord,
} from "@/lib/loss";

export const Route = createFileRoute("/_authenticated/returns/$id")({
  head: () => ({ meta: [{ title: "Return Details — Motion IT BD" }] }),
...
function ReturnDetailsPage() {
  ...
  const load = useCallback(async () => {
    const rec = await fetchReturn(id).catch(() => null);
    if (!rec) { setNotFound(true); setLoading(false); return; }
    setRecord(rec);
    const [rs, cats, atts, evts, fch] = await Promise.all([
      fetchReturnReasons(true).catch(() => [] as ReturnReason[]),
      fetchCategories(true).catch(() => [] as ExpenseCategory[]),
      fetchReturnAttachments(id).catch(() => [] as LossAttachment[]),
      fetchReturnEvents(id).catch(() => [] as LossEvent[]),
      fetchFieldChanges("return", id).catch(() => [] as FieldChange[]),
    ]);
    setReasons(rs); setCategories(cats); setAttachments(atts); setEvents(evts); setChanges(fch);
    const ids = [
  ...
  if (!canAccessModule("returns")) {
    return (<div className="space-y-8"><PageHeader title="Return Details" /><NoAccess /></div>);
  }
  if (loading) {
    return (
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card/50 px-6 py-20 text-center">
          <span className="flex h-14 w-14 items-center justify-center rounded-md bg-muted text-muted-foreground"><Undo2 className="h-7 w-7" /></span>
          <h2 className="mt-5 text-lg font-medium text-foreground">Return not found</h2>
          <p className="mt-2 max-w-md text-sm text-muted-foreground">This return may have been removed or you don't have access to it.</p>
          <Button className="mt-5" asChild><Link to="/returns">Back to all returns</Link></Button>
        </div>
      </div>
    );
  }
  ...
  const canApprove = isAdmin || can("returns", "approve");
  const isLocked = record.status === "approved";
  const canEdit = isLocked ? canApprove : isAdmin || (isOwner && can("returns", "edit"));
  const canComment = isAdmin || can("returns", "view") || isOwner;
  const isOpenForReview = ["submitted", "pending_approval", "revision_requested"].includes(record.status);
  const reasonName = record.reason_id ? reasons.find((r) => r.id === record.reason_id)?.name : null;
  const catName = record.category_id ? categories.find((c) => c.id === record.category_id)?.name : null;
  ...
  async function moveToDeleted() {
    ...
    setBusy(true);
    try {
      await setLossStatus("return", record.id, "deleted", user.id, record.status);
      toast.success("Return moved to deleted.");
      load();
  ...
  async function addAttachment(value: AttachmentValue | null) {
    ...
    try { await addReturnAttachment(record.id, value, user.id); toast.success("Attachment added."); load(); }
  ...
  async function deleteAttachment(att: LossAttachment) {
    try {
      ...
      await deleteLossAttachment("return", att.id);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" asChild><Link to="/returns"><ArrowLeft className="h-4 w-4" />Back</Link></Button>
        ...
        <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {canApprove && isOpenForReview && (
              <LossApprovalPanel kind="return" record={{ id: record.id, number: record.return_number, status: record.status }} onDone={load} />
            )}
            {canEdit && record.status !== "deleted" && (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Move this return to Deleted?</AlertDialogTitle>
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card>
            ...
            <CardContent className="grid gap-x-6 gap-y-4 sm:grid-cols-2">
              <Field label="Return number" value={record.return_number} />
              <Field label="Return date" value={formatDate(record.return_date)} />
              <Field label="Product" value={record.product_name || "—"} />
              <Field label="Quantity" value={String(record.quantity)} />
              <Field label="Reason" value={reasonName ?? "—"} />
              <Field label="Category" value={catName ?? "—"} />
              <Field label="Loss amount" value={formatTk(record.loss_amount)} />
              <Field label="Recoverable amount" value={formatTk(record.recoverable_amount)} />
              <div className="sm:col-span-2">
                <p className="text-xs text-muted-foreground">Net return loss</p>
                <p className="mt-0.5 text-2xl font-semibold tabular-nums text-brand-gradient">{formatTk(record.net_loss_amount)}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div><p className="text-xs text-muted-foreground">Customer notes</p><p className="mt-1 whitespace-pre-wrap text-sm text-foreground">{record.customer_notes || "—"}</p></div>
              <div><p className="text-xs text-muted-foreground">Internal notes</p><p className="mt-1 whitespace-pre-wrap text-sm text-foreground">{record.notes || "—"}</p></div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent><LossDiscussion kind="return" recordId={record.id} comments={commentEvents} names={names} canComment={canComment} onPosted={load} /></CardContent>
          </Card>
        </div>
...
function EditReturnDialog({ open, onClose, record, reasons, categories, onSaved }: { open: boolean; onClose: () => void; record: ReturnRecord; reasons: ReturnReason[]; categories: ExpenseCategory[]; onSaved: () => void; }) {
  const [form, setForm] = useState<ReturnFormValues>(toForm(record));
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      onSaved();
    } catch (err) { toast.error(err instanceof Error ? err.message : "Failed to update return."); }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader><DialogTitle>Edit {record.return_number}</DialogTitle></DialogHeader>
        <form onSubmit={handleSave} className="space-y-5">
          <ReturnFields value={form} onChange={(p) => setForm((f) => ({ ...f, ...p }))} reasons={reasons} categories={categories} extraStatuses={[record.status]} />
          <DialogFooter>
...
function toForm(record: ReturnRecord): ReturnFormValues {
  return {
    ...
    loss_amount: String(record.loss_amount ?? ""),
    recoverable_amount: String(record.recoverable_amount ?? ""),
    notes: record.notes ?? "",
