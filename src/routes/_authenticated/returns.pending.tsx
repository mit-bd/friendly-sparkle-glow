...
import { fetchReturnReasons, formatTk, type ReturnReason, type ReturnRecord } from "@/lib/loss";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const db = supabase as any;

export const Route = createFileRoute("/_authenticated/returns/pending")({
  head: () => ({ meta: [{ title: "Pending Returns — Motion IT BD" }] }),
...
type SortField = "return_number" | "return_date" | "net_loss_amount" | "submitted_at";
...
function PendingReturnsPage() {
  ...
  useEffect(() => {
    fetchReturnReasons(true).then(setReasons).catch(() => undefined);
  }, []);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    let q = db.from("returns").select("*", { count: "exact" });
    if (statusFilter !== ALL) q = q.eq("status", statusFilter);
    else q = q.in("status", QUEUE_STATUSES);
    if (reasonFilter !== ALL) q = q.eq("reason_id", reasonFilter);
    const s = search.replace(/[(),]/g, " ").trim();
    if (s) {
      const like = `%${s}%`;
      q = q.or(`return_number.ilike.${like},product_name.ilike.${like},notes.ilike.${like}`);
    }
    q = q.order(sortField, { ascending: sortAsc, nullsFirst: false });
    q = q.range(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE - 1);
    const { data, count, error } = await q;
    if (error) { toast.error(error.message); setLoading(false); return; }
    const list = (data ?? []) as ReturnRecord[];
    setRows(list);
    setTotal(count ?? 0);
    setNames(await fetchUserNames(list.map((r) => r.submitted_by ?? r.created_by ?? "")));
    setLoading(false);
  }, [statusFilter, reasonFilter, search, sortField, sortAsc, page]);

  useEffect(() => { if (canApprove) load(); }, [load, canApprove]);

  if (!canApprove) {
    return (<div className="space-y-8"><PageHeader title="Pending Returns" /><NoAccess /></div>);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Pending Returns" description="Review, approve, reject, or request revisions on submitted returns." />
          ...
          <Card>
            <CardContent className="grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="flex items-end">
                <Button variant="ghost" onClick={() => { setStatusFilter(ALL); setReasonFilter(ALL); setPage(0); }}>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <SortHead label="Return No." field="return_number" {...{ sortField, sortAsc, toggleSort }} />
                  <SortHead label="Date" field="return_date" {...{ sortField, sortAsc, toggleSort }} />
                  <TableHead>Product</TableHead>
                  <TableHead>Reason</TableHead>
                  <SortHead label="Net loss" field="net_loss_amount" className="text-right" {...{ sortField, sortAsc, toggleSort }} />
              ...
              <TableBody>
                {loading ? (
                  ...
                  rows.map((r) => {
                    ...
                    return (
                      <TableRow key={r.id} className="cursor-pointer" onClick={() => navigate({ to: "/returns/$id", params: { id: r.id } })}>
                        <TableCell className="font-medium">{r.return_number}</TableCell>
                        <TableCell className="text-muted-foreground">{formatDate(r.return_date)}</TableCell>
                        <TableCell className="max-w-[160px] truncate">{r.product_name || "—"}</TableCell>
                        <TableCell className="text-muted-foreground">{r.reason_id ? reasonMap.get(r.reason_id) ?? "—" : "—"}</TableCell>
                        ...
                        <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-2">
                            <LossApprovalPanel kind="return" record={{ id: r.id, number: r.return_number, status: r.status }} onDone={load} />
                            <Button variant="ghost" size="icon" aria-label="View return" asChild>
                              <Link to="/returns/$id" params={{ id: r.id }}><Eye className="h-4 w-4" /></Link>
                            </Button>
                          </div>
                        </TableCell>
                      </TableRow>
